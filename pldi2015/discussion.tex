...
\code{mean(D::Distribution)}. In contrast, much scientific code is written in
...
\begin{tabular}{l || l | l | l | l}
  \code{D}                & \code{pdf(D)}    & \code{cdf(D)}    & \code{quantile(D)} & \code{rand(D)} \\
  \hline
	\code{Beta}             & \code{dbeta}     & \code{pbeta}     & \code{qbeta}       & \code{rbeta}    \\
	\code{Binomial}         & \code{dbinom}    & \code{pbinom}    & \code{qbinom}      & \code{rbinom}   \\
	\code{Cauchy}           & \code{dcauchy}   & \code{pcauchy}   & \code{qcauchy}     & \code{rcauchy}  \\
	\code{Chisq}            & \code{dchisq}    & \code{pchisq}    & \code{qchisq}      & \code{rchisq}   \\
	\code{Exponential}      & \code{dexp}      & \code{pexp}      & \code{qexp}        & \code{rexp}     \\
	\code{Fdist}            & \code{df}        & \code{pf}        & \code{qf}          & \code{rf}       \\
	\code{Gamma}            & \code{dgamma}    & \code{pgamma}    & \code{qgamma}      & \code{rgamma}   \\
	\code{Geometric}        & \code{dgeom}     & \code{pgeom}     & \code{qgeom}       & \code{rgeom}    \\
	\code{Hypergeometric}   & \code{dhyper}    & \code{phyper}    & \code{qhyper}      & \code{rhyper}   \\
	\code{LogNormal}        & \code{dlnorm}    & \code{plnorm}    & \code{qlnorm}      & \code{rlnorm}   \\
	\code{Multinomial}      & \code{dmultinom} & \code{pmultinom} & \code{qmultinom}   & \code{rmultinom}\\
	\code{NegativeBinomial} & \code{dnbinom}   & \code{pnbinom}   & \code{qnbinom}     & \code{rnbinom}  \\
	\code{Normal}           & \code{dnorm}     & \code{pnorm}     & \code{qnorm}       & \code{rnorm}    \\
	\code{Poisson}          & \code{dpois}     & \code{ppois}     & \code{qpois}       & \code{rpois}    \\
	\code{TDist}            & \code{dt}        & \code{pt}        & \code{qt}          & \code{rt}       \\
	\code{Uniform}          & \code{dunif}     & \code{punif}     & \code{qunif}       & \code{runif}    \\
	\code{Weibull}          & \code{dweibull}  & \code{pweibull}  & \code{qweibull}    & \code{rweibull} \\
\end{tabular}
}

List of common distributions in R v3.1.0 and their associated factorization into a common interface
of generic functions and \code{Distribution} types in the \code{Distributions.jl} Julia package.
