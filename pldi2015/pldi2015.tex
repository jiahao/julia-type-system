...
Julia types can also take parameters which are also
types. 
...
An unqualified \verb|TypeVar| can be omitted from a parametric type declaration.
For example, \verb|Array| is a synonym for \verb|Array{T,N}| for some element
type \verb|T| and rank \verb|N|, and is convenient shorthand for writing methods
that are agnostic about these type parameters.
...
\TODO{Say more about how this makes sense for arrays in terms to determining
what types can be put in the array.}
...
A second example: the three \code{Complex} numbers
...
The type of the result \code{a + b} depends on the types of \code{a} and
\code{b} via \code{lcm}.
