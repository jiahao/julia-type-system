...
performs extensive static optimizations such as automatic type inference, tuple
elimination and inlining, on top of the compiler optimizations accessible
through LLVM. When combined with an extensive base library for parallel
...
often within a factor of two of the speed of native, hand-written C code. This
paper aims to explain how Julia's language constructs and compiler
...
practice requiring multiple dispatch to be emulated using visitor patterns.
Furthermore, implementing binary methods can require knowing the internal
representation of both objects \verb|a| and \verb|b|, especially for
performance reasons~\cite{Bruce1995}. Such knowledge fundamentally corrupts the
abstraction of class-based encapsulation, as the methods associated with
\verb|a| must know implementation details of all possible objects \verb|b| that
\verb|a| may interact with. In contrast, there is no conceptual problem with
allowing a generic function \verb|*| knowledge about the internal
representations of the types it works on.
