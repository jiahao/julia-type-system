...
\begin{abstract}
  ...
  allowing highly productive experimentation.
  ...
  polymorphic behviors using dynamic multiple dispatch over
...
know what computations they need to perform at the outset of their word.
...
These restrictions can occasionally conflict with how a user would
prefer to think about a problem. In fact this is especially likely to happen
when an answer can be justified by specialized domain knowledge that
...
Programs are not required to be checked for provable correctness,
but rather they are executed until the program terminates or a runtime error occurs.
...
to form a lattice structure, which enables effective data flow analysis.
...
Unusually, Julia's type system tries to hide type kinds from
...
function g()
    ...
    x + x #inferred type is Complex{Float64}
end
\end{lstlisting}
%
