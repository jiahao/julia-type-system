...
The resulting algorithm for determining a useful matrix factorization for solving a
set of linear equations, $Ax=b$, has to
...
additional cost of doing so is not prohibitive. This can be illustrated by Julia's
\texttt{\textbackslash} function. The user has the option of choosing a specific
interpretation for the left hand side matrix $A$ and use that information for dispatch.
In case the matrix is positive definite by \texttt{cholfact(A)\textbackslash b} or if 
triangular simply by \texttt{Triangular(A)\textbackslash b}

When no information is provided, i.e. \texttt{A\textbackslash b}, the left hand side is
analyzed for structure. The code takes the following form:
...
The matrix is initially checked for being square and, if not, the matrix \texttt{A}
is QR factorized and the least squares problem $\min_x\|b-Ax\|$ is solved. When
the matrix \texttt{A} is square, it is checked for being triangular and the if is
found to be so, the structure is exploited to solve $Ax=b$. If the matrix is not found to be
triangular, it is LU factorized and the method dispatches to solver specialized for LU factorizations.
It is possible to check for symmetry and positive definiteness, but these checks are considered too
...
certain functions like \code{\textbackslash} wish to combine them.
% It should be
