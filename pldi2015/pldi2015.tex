...
become popular amongst technical computing users for writing prototype
...
in a static language. The two-language problem can be viewed as an unavoidable
...
corresponding array \verb|Array{Complex| \verb|{Float64}, N}|. The vectorized
methods enable convenient syntax for mapping over many scalar values, but can incur
unwanted allocation of array temporaries. In this example, \verb|h1| constructs
three temporary arrays on top of the new array needed to contain the answer,
namely the arrays \verb|A|, \verb|B| and \verb|C| in the equivalent code: 
...
composability. The generic function \verb|h1| has methods with function
types
...
the standard library are implemented in a consistent fashion using code
...
code with explicit loops. Alternatively, a more general approach is to use
higher order functions like \verb|map| to vectorize an entire expression, like
so:
